...
const [user] = useState(() => {
  return JSON.parse(
    localStorage.getItem("inhouseUser")
  );
});
const rol =
  user?.user?.rol ||
...
const autorizarContrata = async (
  ...
  try {
    ...
    const res = await api.put(
      `/contratasinhouse/${contrata.id}/autorizar`,
      {
        accionPorId: user?.id
      }
    );
...
const aprobarSeguridad = async (contrata) => {
  try {
    const res = await api.put(
      `/contratasinhouse/${contrata.id}/aprobar-seguridad`,
      {
        usuarioId: user?.id
      }
    );
...
const rechazarSeguridad = async (contrata) => {
  ...
  try {
    const res = await api.put(
      `/contratasinhouse/${contrata.id}/rechazar`,
      {
        usuarioId: user?.id,
...
const aprobarAmbiente = async (contrata) => {
  const res = await api.put(
    `/contratasinhouse/${contrata.id}/aprobar-ambiente`,
    {
      usuarioId: user?.id
    }
  );
